...
class student:
  ...
  def new_student(self,email,phone):
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS sdirectory(name TEXT NOT NULL PRIMARY KEY,password,email,phone,notification TEXT NOT NULL DEFAULT '',orders_part_of TEXT NOT NULL DEFAULT '');")
    ...
    #this is in a try except statement because if the data isn't correct then we need to send the user an error message
      try:
        cur.execute("INSERT INTO sdirectory(name,password,email,phone) VALUES(?,?,?,?);",(self.name,self.password,email,phone))
        conn.commit()
        return True
      except:
  ...
  def login(self):
    ...
    cur.execute("SELECT * FROM sdirectory WHERE name IS ? AND password IS ?;",(self.name,self.password))
